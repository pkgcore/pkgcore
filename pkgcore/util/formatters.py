...
class TerminfoColor(object):
    ...
    def __call__(self, formatter):
        if self.color is None:
            ...
            res = curses.tparm(formatter._setColor[self.mode], color)
...
class TerminfoFormatter(Formatter):
    ...
    def __init__(self, stream=None, term=None):
        ...
        if stream is None:
            stream = sys.stdout
        self.stream = stream
        curses.setupterm(fd=stream.fileno(), term=term)
        ...
        self._colorReset = curses.tigetstr('op')
        self._setColor = (curses.tigetstr('setaf'), curses.tigetstr('setab'))
        # [fg, bg]
