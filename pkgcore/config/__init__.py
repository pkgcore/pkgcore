...
def load_config(
	local_conf_file=DEFAULT_CONF_FILE, global_conf_file=GLOBAL_CONF_FILE,
	...
	if os.path.isfile(global_conf_file) and os.path.isfile(local_conf_file):
		...
		raise errors.BaseException(
			"sorry, config file %r or %r doesn't exist, and I don't like "
			"make.conf currently (I'm working out my issues however)" %
			(local_conf_file, global_conf_file))
	return c
