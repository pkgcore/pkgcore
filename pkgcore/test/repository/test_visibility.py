...
class TestVisibility(unittest.TestCase):
	...
	def test_filtering(self):
		...
		self.assertEqual(sorted(self.vrepo.itermatch(a)), sorted(self.repo.itermatch(a)))
		self.assertEqual(sorted(self.vrepo.itermatch(a2)), sorted([]))
		self.setup_repos(atom("=dev-util/diffball-1.0"))
		self.assertEqual(sorted(self.vrepo.itermatch(a)), sorted(self.repo.itermatch(a)))
		self.assertEqual(sorted(self.vrepo.itermatch(a2)), sorted([CPV("dev-util/diffball-0.7")]))
		self.setup_repos(packages.PackageRestriction("package", values.OrRestriction(*[values.StrExactMatch(x) for x in ("diffball", "fake")])))
		self.assertEqual(sorted(self.vrepo.itermatch(packages.AlwaysTrue)), sorted(self.repo.itermatch(atom("dev-util/bsdiff"))))

	def test_iter(self):
		self.setup_repos(packages.PackageRestriction("package", values.OrRestriction(*[values.StrExactMatch(x) for x in ("diffball", "fake")])))
		self.assertEqual(sorted(self.vrepo), sorted(self.repo.itermatch(atom("dev-util/bsdiff"))))
