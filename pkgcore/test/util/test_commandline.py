...
class TestExtendedRestrictionGeneration(unittest.TestCase):
	...
	def verify_text_glob(self, restrict, token):
		self.assertTrue(isinstance(restrict, values.StrGlobMatch), msg="isinstance testing for %r against %r" % (token, restrict))
		self.assertEqual(restrict.prefix, token.endswith("*"), msg="testing for %r against %r: val %r" % (token, restrict, restrict.prefix))
		self.assertEqual(restrict.glob, token.strip("*"), msg="verifying the restriciton glob, %r %r" % (restrict.glob, token))
	
	def verify_text(self, restrict, token):
		self.assertTrue(isinstance(restrict, values.StrExactMatch), msg="verifying restrict %r from %r is StrExactMatch" % (restrict, token))
		self.assertEquals(restrict.exact, token)
	
	def verify_text_containment(self, restrict, token):
		self.assertTrue(isinstance(restrict, values.ContainmentMatch), msg="verifying restrict %r from %r is ContainmentMatch" % (restrict, token))
	...
	def test_convert_glob(self):
		...
		for token in ("*", "**", ""):
			...
			self.assertEquals(i, None, msg="verifying None is returned on pointless restrictions")

		for token in ("*diff*", "*b*"):
			self.verify_text_containment(convert_glob(token), token)

	def verify_restrict(self, restrict, attr, token):
		self.assertTrue(isinstance(restrict, packages.PackageRestriction), msg="isinstance verification of %r %r" % (token, restrict))
		self.assertEqual(restrict.attr, attr, msg="verifying package attr %r; required(%s), token %s" % (restrict.attr, attr, token))
	...
	def test_combined(self):
		self.assertTrue(isinstance(generate_restriction("dev-util/diffball"), atom))
