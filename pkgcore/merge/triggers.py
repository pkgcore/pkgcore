...
__all__ = [
	"trigger", "SimpleTrigger", "merge_trigger", "unmerge_trigger",
	"ldconfig_trigger"]
...
class trigger(object):
		...
		if not isinstance(cset_name, basestring):
			for x in cset_name:
				if not isinstance(x, basestring):
					raise TypeError(
						"cset_name must be either a list, "
						"or a list of strings: %s" % x)
		else:
			cset_name = [cset_name]
	...
	def __repr__(self):
		...
		return "<%s cset=%r ftrigger=%r %s@#%x>" % (
			self.__class__.__name__,
			self.required_csets, self.trigger, s, id(self))
...
class SimpleTrigger(trigger):
	...
	def __init__(self, cset_name, ftrigger, register_func=None, label=None):
		...
		if not isinstance(cset_name, (list, tuple)):
			...
			cset_name = [cset_name]
		trigger.__init__(
			self, cset_name, ftrigger, register_func=register_func,
			label=label)
...
def fix_default_gid(gid=pkgcore.os_data.portage_gid,
					replacement=pkgcore.os_data.root_gid, cset="new_cset"):
	def change_gid(engine, cset):
		# do it as a list, since we're mutating the set
		resets = [
			x.change_attributes(gid=replacement) for x in cset if x.gid == gid]
		cset.update(resets)
	return SimpleTrigger(cset, change_gid)

def fix_default_uid(uid=pkgcore.os_data.portage_uid,
